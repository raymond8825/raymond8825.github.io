---
title: ElasticSearch进阶：ES查询在Java中的实现
slug: elasticsearch-advanced-es-query-s-implementation-in-java-1s3nq5
url: /post/elasticsearch-advanced-es-query-s-implementation-in-java-1s3nq5.html
date: '2023-10-30 15:34:48'
lastmod: '2023-11-02 12:55:49'
toc: true
categories:
  - Elasticsearch
isCJKLanguage: true
...
# ElasticSearch进阶：ES查询在Java中的实现

---

* [https://mp.weixin.qq.com/s/7vEy-vN8JV3o6sAh6HFohA - 微信公众平台](https://mp.weixin.qq.com/s/7vEy-vN8JV3o6sAh6HFohA)
* 2023-10-30 15:34:48

---

这篇博文的主题是 ES 的查询，因此我整理了尽可能齐全的 ES 查询场景，形成下面的图：

​![图片](http://127.0.0.1:6806/assets/640-20231030153448-wf7wl5q.png)​

本文基于 elasticsearch 7.13.2 版本，es 从 7.0 以后，发生了很大的更新。7.3 以后，已经不推荐使用 TransportClient 这个 client，取而代之的是 Java High Level REST Client。

#### 测试使用的数据示例

首先是，Mysql 中的部分测试数据：

​![图片](http://127.0.0.1:6806/assets/640-20231030153448-siny8d2.png)​

Mysql 中的一行数据在 ES 中以一个文档形式存在：
...
简单梳理了一下 ES JavaAPI 的相关体系，感兴趣的可以自己研读一下源码。

​![图片](http://127.0.0.1:6806/assets/640-20231030153448-sa4jxwf.png)​

接下来，我们用十几个实例，迅速上手 ES 的查询操作，每个示例将提供 SQL 语句、ES 语句和 Java 代码。

### 1 词条查询

所谓词条查询，也就是 ES 对查询条件不会进行分词处理，只有当词条和查询字符串完全匹配时，才会被查询到。
...
而使用 ES 查询语句却很不一样（注意查询字段带上 keyword）：
...
ElasticSearch 5.0 以后，string 类型有重大变更，移除了 string 类型，string 字段被拆分成两种新的数据类型: `text`​ 用于全文搜索的，而 `keyword`​ 用于关键词搜索。
...
仔细观察查询结果，会发现 ES 查询结果中会带有 `_score`​ ​这一项，ES 会根据结果匹配程度进行评分。打分是会耗费性能的，如果确认自己的查询不需要评分，就设置查询语句关闭评分：
...
​![图片](http://127.0.0.1:6806/assets/640-20231030153448-x0l11v8.png)​

bool 过滤器下可以有 4 种子条件，可以任选其中任意一个或多个。filter 是比较特殊的，这里先不说。
...
* should：至少有 n 个语句要匹配，n 由参数控制。

<span style="font-weight: bold;" data-type="strong">精度控制：</span>
...
        .minimumShouldMatch(1);  // 设置should至少需要满足几个条件// 将BoolQueryBuilder构建到SearchSourceBuilder中
...
<span style="font-weight: bold;" data-type="strong">方式一，单独使用：</span>
...
<span style="font-weight: bold;" data-type="strong">方式二，和 must、must_not 同级，相当于子查询：</span>
...
<span style="font-weight: bold;" data-type="strong">方式三，将 must、must_not 置于 filter 下，这种方式是最常用的：</span>
...
    System.out.println("searchSourceBuilder----->" + searchSourceBuilder);    searchRequest.source(searchSourceBuilder);
...
使用聚合查询，结果中默认只会返回 10 条文档数据（当然我们关心的是聚合的结果，而非文档）。返回多少条数据可以自主控制：
...
<span style="font-weight: bold;" data-type="strong">案例：查询明教中的最大年龄。这涉及到聚合与条件查询一起使用。</span>
...
<span style="font-weight: bold;" data-type="strong">案例：查询 0-20，21-40，41-60，61 以上的各有多少人。</span>
...
以上是 ElasticSearch 查询的全部内容，丰富详实，堪比操作手册，强烈建议收藏！

> *感谢阅读，希望对你有所帮助 :) *
>
> *来源：blog.csdn.net/mu_wind/article/details/118423362*
